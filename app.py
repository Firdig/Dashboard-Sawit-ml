...
# =========================================================
# 🔗 GOOGLE DRIVE CONNECT (MODIFIKASI DI SINI)
# =========================================================
# Masukkan ID file Google Drive Anda di sini
DRIVE_IDS = {
    "class_names.json": "1WsTipw01xyUspgkgR0B94G7RFL8WLAQs",
    "xgb_hsv.joblib": "1eaPu8Vpq0ZrJ4FOE5IOy806y2iqY4dmm",
    "xgb_meta.joblib": "1sMIazdCUizS1NAI0P-AGB6HU8ca2Z73H",
    "model_effnetb0_lora_merged.keras": "1tZ6wmpd88f0EqaGNOTmcv6oBaOd10S1Y",
    "maxvit_merged.pt": "1h6eQwbMllAslI4I6rIbElkz5Toc69B4Y",
}

def ensure_models_downloaded():
    """Mengecek apakah model ada, jika tidak, download dari GDrive."""
    
    # Pastikan folder tujuan ada
    if not MODELS_DIR.exists():
        MODELS_DIR.mkdir(parents=True, exist_ok=True)

    with st.spinner('Mengecek dan mengunduh model dari Google Drive...'):
        for file_key, drive_id in DRIVE_IDS.items():
            # Tentukan nama file asli sesuai dictionary FILES di kode asli Anda
            # Mapping manual nama file agar sesuai dengan FILES di bawah
            filename = ""
            if file_key == "class_names.json": filename = "class_names.json"
            elif file_key == "xgb_hsv.joblib": filename = "xgb_hsv.joblib"
            elif file_key == "xgb_meta.joblib": filename = "xgb_meta.joblib"
            elif file_key == "model_effnetb0_lora_merged.keras": filename = "model_effnetb0_lora_merged.keras"
            elif file_key == "maxvit_merged.pt": filename = "maxvit_merged.pt"
            
            dest_path = MODELS_DIR / filename
            
            # Jika file belum ada atau ukurannya 0, download
            if not dest_path.exists():
                st.info(f"Sedang mengunduh {filename}...")
                url = f'https://drive.google.com/uc?id={drive_id}'
                try:
                    gdown.download(url, str(dest_path), quiet=False)
                except Exception as e:
                    st.error(f"Gagal mengunduh {filename}. Pastikan ID benar dan akses Public. Error: {e}")
